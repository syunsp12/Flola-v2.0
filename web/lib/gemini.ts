...
export const classifyTransaction = async (description: string, categories: Category[]) => {
  ...
  try {
      ...
      generationConfig: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            normalized_merchant: { type: SchemaType.STRING },
    ...
    const prompt = `You are a Japanese financial assistant. Classify this transaction description into the most relevant category.
...
export const analyzeSalarySlip = async (base64Image: string) => {
  ...
  try {
    const model = ai.getGenerativeModel({ 
      ...
      generationConfig: {
        ...
        responseSchema: {
          type: SchemaType.OBJECT,
          properties: {
            base_pay: { type: SchemaType.NUMBER },
            overtime_pay: { type: SchemaType.NUMBER },
            tax: { type: SchemaType.NUMBER },
            social_insurance: { type: SchemaType.NUMBER },
    ...
    const prompt = `Analyze this salary slip image and extract the following details in JSON format.
